  ...
  return (
    <Container>
     {/* <div style={{
      ...
      {/**/}
      <StyledSvg  className="bombilla" viewBox='0 0 300 150' off={off}>
        <defs>
          <filter id="blur">
      ...
      <button onClick={() => setOff(!off)}>{off ? 'Encender' : 'Apagar'}</button>
    </Container>
  )
}
...
const filamentoOff = off => {
  const color1 = off ? colorOff : colorOn;
  const color2 = off ? colorOn : colorOff;
  return keyframes`
    0%{
      fill:${color1};
    }
...
const StyledSvg = styled.svg`
  width: 100%;
  .bombillo{
    transform: scale(.8) translate(-61px,39px)  rotate(180deg);
  ...
  .halo{
    ...
    transition: all 3s ${props => !props.off ? '3s' : '0s'};
    transform: ${props => props.off ? 'scale(0.2)' : 'scale(1) translateY(-20px)'};
  ...
  .filamento{
    fill: ${colorOff};
    ${({ off }) => css`animation: ${filamentoOff(off)} 5s forwards`};
  }
